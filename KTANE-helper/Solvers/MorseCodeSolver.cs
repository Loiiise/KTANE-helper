...
namespace KTANE_helper.Solvers
{
    internal class MorseCodeSolver : Solvable<MorseCodeSolver>
    {
        ...
        internal override void Solve(BombKnowledge _)
        {
            ...
            Show("Use . for shorts and - or , for longs.");
            string answer;
            while (!ProcessLetter(Query("What is a letter you see?"), out answer));

            if (answer is not null) Show($"The correct frequency is {words[answer]} MHz");
            else
            {
                Show("you f'ed up!");
                Solve(_);
